...
FlattenLayer::FlattenLayer(size_t imageHeight, size_t imageWidth,
                           size_t imageDepth)
    : height(imageHeight), width(imageWidth), depth(imageDepth) {
  ...
  prevLayerGrad.resize(imageDepth);
  for (size_t i = 0; i < imageDepth; i++) {
    prevLayerGrad[i].resize(imageHeight);
    for (size_t j = 0; j < imageHeight; j++) {
      prevLayerGrad[i][j].assign(imageWidth, 0.0);
    }
  }
}
...
void FlattenLayer::backwardProp_batch(vector<double> &nextLayerGrad) {
  backwardProp(nextLayerGrad); // Same logic as single backwardProp because
                               // there are no weights to accumulate
}
