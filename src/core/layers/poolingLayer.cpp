...
//Note:                 N/A
poolingLayer::poolingLayer(poolKernel& kernelConfig, featureMapDim& FM_Dim, poolingLayerType& poolType)
...
void poolingLayer::forwardProp(vector<featureMapType>& inputFeatureMaps)
{
        ...
        size_t out_FM_width_Iter = 0;   //iterates the columns of the output feature map

        //iterate on every channel (depth) using 'd', where 'd' is the depth corrisponding
        //of the kernel depth and the input feature map depth, and output feature map depth
        for(size_t d = 0; d < kernel_info.filter_depth; d++)
        {
                //moving on the 2D feature map the length to be moved is fm_dim - krnl_dim + 1

                //move along the rows of the input feature map using 'i', and jump with amount "stride"
                for(size_t i = 0; i <(inputFeatureMaps[d].size() - kernel_info.filter_height + 1); i += kernel_info.stride)
                {
                        //move along the columns of the input feature map using 'j', and jump with amount "stride"
                        for(size_t j = 0; j < (inputFeatureMaps[d][i].size() - kernel_info.filter_width + 1); j += kernel_info.stride)
                        {
