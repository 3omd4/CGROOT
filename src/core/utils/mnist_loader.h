...
public:
   ...
   */
  static std::unique_ptr<MNISTDataset>
  load_training_data(const std::string &images_path,
   ...
   */
  static std::unique_ptr<MNISTDataset>
  load_test_data(const std::string &images_path,
                 const std::string &labels_path);
   ...
   */
  static void one_hot_encode_labels(MNISTDataset &dataset,
   ...
   */
  static std::vector<std::vector<MNISTImage>>
  create_batches(const MNISTDataset &dataset, size_t batch_size);
...
private:
   ...
   */
  static std::unique_ptr<MNISTDataset>
  load_dataset(const std::string &images_path, const std::string &labels_path,
               uint32_t images_magic, uint32_t labels_magic);
 ...
 */
void shuffle_dataset(MNISTLoader::MNISTDataset &dataset,
...
void save_to_csv(const MNISTLoader::MNISTDataset &dataset,
                 const std::string &filename, size_t max_samples = 0);
