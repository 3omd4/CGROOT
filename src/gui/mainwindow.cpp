...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), m_tabWidget(nullptr), m_trainingWidget(nullptr),
      m_inferenceWidget(nullptr), m_configWidget(nullptr),
      m_metricsWidget(nullptr), m_logOutput(nullptr), m_progressBar(nullptr),
      m_statusLabel(nullptr), m_workerThread(nullptr), m_updateTimer(nullptr) {
...
void MainWindow::setupMenuBar() {
  ...
  m_loadDatasetAction =
      fileMenu->addAction("&Load Dataset...", this, &MainWindow::onLoadDataset);
  ...
  m_startTrainingAction = trainingMenu->addAction("&Start Training", this,
                                                  &MainWindow::onStartTraining);
  m_stopTrainingAction = trainingMenu->addAction("S&top Training", this,
                                                 &MainWindow::onStopTraining);
  m_stopTrainingAction->setEnabled(false);

  QMenu *inferenceMenu = menuBar()->addMenu("&Inference");
  m_startInferenceAction = inferenceMenu->addAction(
      "Run &Inference", this, &MainWindow::onStartInference);

  QMenu *helpMenu = menuBar()->addMenu("&Help");
  helpMenu->addAction("&About", this, [this]() {
    QMessageBox::about(this, "About",
                       "Fashion-MNIST Neural Network Trainer\n\n"
                       "A comprehensive GUI for training and testing neural "
                       "networks on Fashion-MNIST dataset.");
  });
}
...
void MainWindow::createConnections() {
  if (m_modelController) {
    connect(m_modelController.get(), &ModelController::metricsUpdated, this,
            &MainWindow::onUpdateMetrics);
    connect(m_modelController.get(), &ModelController::progressUpdated, this,
            &MainWindow::onUpdateProgress);
    connect(m_modelController.get(), &ModelController::logMessage, this,
            &MainWindow::onLogMessage);
    connect(m_modelController.get(), &ModelController::imagePredicted, this,
            &MainWindow::onImagePrediction);
    connect(m_modelController.get(), &ModelController::trainingFinished, this,
            &MainWindow::onTrainingFinished);
    connect(m_modelController.get(), &ModelController::modelStatusChanged, this,
            &MainWindow::onModelStatusChanged);
  }

  if (m_trainingWidget) {
    connect(m_trainingWidget, &TrainingWidget::startTrainingRequested, this,
            &MainWindow::onStartTraining);
    connect(m_trainingWidget, &TrainingWidget::stopTrainingRequested, this,
            &MainWindow::onStopTraining);
  }
...
void MainWindow::onLoadDataset() {
  QString imagesPath = QFileDialog::getOpenFileName(
      this, "Select Fashion-MNIST Images File", "",
      "MNIST Files (*.idx3-ubyte);;All Files (*.*)");
  if (imagesPath.isEmpty())
    return;

  QString labelsPath = QFileDialog::getOpenFileName(
      this, "Select Fashion-MNIST Labels File", "",
      "MNIST Files (*.idx1-ubyte);;All Files (*.*)");
  if (labelsPath.isEmpty())
    return;

  if (m_modelController) {
    m_modelController->loadDataset(imagesPath.toStdString(),
...
void MainWindow::onLoadModel() {
  QString filePath = QFileDialog::getOpenFileName(
      this, "Load Model", "", "Model Files (*.model);;All Files (*.*)");
  if (filePath.isEmpty())
    return;
...
void MainWindow::onSaveModel() {
  QString filePath = QFileDialog::getSaveFileName(
      this, "Save Model", "", "Model Files (*.model);;All Files (*.*)");
  if (filePath.isEmpty())
    return;
