...
// Helper to convert std::vector<uint8_t> from MNISTImage to Python buffer protocol if needed
// Or just let pybind11 convert to list.
...
void bind_definitions(py::module &m) {
    ...
    py::class_<architecture>(m, "architecture")
        ...
        .def_readwrite("kernelsPerconvLayers", &architecture::kernelsPerconvLayers)
        .def_readwrite("neuronsPerFCLayer", &architecture::neuronsPerFCLayer)
        .def_readwrite("convLayerActivationFunc", &architecture::convLayerActivationFunc)
        .def_readwrite("FCLayerActivationFunc", &architecture::FCLayerActivationFunc)
        .def_readwrite("convInitFunctionsType", &architecture::convInitFunctionsType)
        .def_readwrite("FCInitFunctionsType", &architecture::FCInitFunctionsType)
        .def_readwrite("distType", &architecture::distType)
        .def_readwrite("poolingLayersInterval", &architecture::poolingLayersInterval)
        .def_readwrite("poolingtype", &architecture::poolingtype)
        .def_readwrite("kernelsPerPoolingLayer", &architecture::kernelsPerPoolingLayer);
}

void bind_mnist_loader(py::module &m) {
    // We bind the data structures. 
    // Since 'image' is vector<vector<vector<unsigned char>>>, pybind11 handles this automatically 
    // via stl.h, providing list of lists.
    ...
    py::class_<cgroot::data::MNISTLoader::MNISTDataset>(m, "MNISTDataset")
        ...
        .def_readwrite("num_images", &cgroot::data::MNISTLoader::MNISTDataset::num_images)
        .def_readwrite("image_width", &cgroot::data::MNISTLoader::MNISTDataset::image_width)
        .def_readwrite("image_height", &cgroot::data::MNISTLoader::MNISTDataset::image_height);

    // Bind MNISTLoader static methods
    py::class_<cgroot::data::MNISTLoader>(m, "MNISTLoader")
        .def_static("load_training_data", &cgroot::data::MNISTLoader::load_training_data)
