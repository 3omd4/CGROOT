...
def build_configuration(cmake_path, config, compiler_name):
    ...
    # Require BOTH Ninja and vcvars to use Ninja generator with MSVC
    # Otherwise fallback to Visual Studio generator which handles environment automatically
    vcvars = find_vcvars_bat()
    
    if get_os_type() == "windows" and ninja_path and vcvars:
        use_ninja = True
    ...
    if os_type == "windows":
        if use_ninja:
            ...
            if vcvars:
                ...
                # Use 'call' for safety in cmd execution chain
                cmd_configure = f'call "{vcvars}" && {cmd_configure}'
                cmd_build = f'call "{vcvars}" && {cmd_build}'
            else:
            ...
            print(f"{GREEN}Using Generator: {generator}{RESET}")
